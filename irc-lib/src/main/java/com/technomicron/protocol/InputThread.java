 ...
 * passes these lines to the IRCClient without changing them.
...
public class InputThread extends Thread {
     ...
     * IRCClient to handle them.
     *
     * @param bot An instance of the underlying IRCClient.
     ...
     */
    public InputThread(IRCClient bot, Socket socket, BufferedReader breader, BufferedWriter bwriter) {
        _bot = bot;
        _socket = socket;
        _breader = breader;
        _bwriter = bwriter;
        this.setName("QC Input");
    }
     ...
     * in the IRCClient, which may subsequently call an 'onXxx' method
     * in the IRCClient subclass.  If any subclass of Throwable (i.e.
     * any Exception or Error) is thrown by your method, then this
     * method will print the stack trace to the standard output.  It
     * is probable that the IRCClient may still be functioning normally
                ...
                try {
                    ...
                    while ((line = _breader.readLine()) != null) {
                        try {
                            ...
                            synchronized (_bot) {
                                _bot.log("### Your implementation of IRCClient is faulty and you have");
                                _bot.log("### allowed an uncaught Exception or Error to propagate in your");
                                _bot.log("### code. It may be possible for IRCClient to continue operating");
