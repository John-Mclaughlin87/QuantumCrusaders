...
This file is part of IRCClient.
...
package com.technomicron.protocol;
...
/**
 * IRCClient is a Java framework for writing IRC bots quickly and easily.
 *  <p>
 ...
 * channel statistics.
 *  <p>
 * Methods of the IRCClient class can be called to send events to the IRC server
 ...
 * can be supported using multiple instances of IRCClient.
 *  <p>
 * To perform an action when the IRCClient receives a normal message from the IRC
 * server, you would override the onMessage method defined in the IRCClient
 * class.  All on<i>XYZ</i> methods in the IRCClient class are automatically called
 ...
 * methods are already correctly implemented in the IRCClient and should not
 * normally need to be overridden.  Please read the full documentation for each
 * method to see which ones are already implemented by the IRCClient class.
 *  <p>
 * Please visit the IRCClient homepage at
 * <a href="http://www.jibble.org/pircbot.php">http://www.jibble.org/pircbot.php</a>
 * for full revision history, a beginners guide to creating your first IRCClient
 * and a list of some existing Java IRC bots and clients that use the IRCClient
 * framework.
 * 
...
public abstract class IRCClient implements ReplyConstants {
    ...
    /**
     * Constructs a IRCClient with the default settings.  Your own constructors
     * in classes which extend the IRCClient abstract class should be responsible
    ...
    public synchronized void connect(String hostname, int port, String password) throws IOException, IrcException, NickAlreadyInUseException {
        ...
        if (isConnected()) {
            throw new IOException("The IRCClient is already connected to an IRC server.  Disconnect first.");
        }
        ...
        while ((line = breader.readLine()) != null) {
            ...
            if (secondSpace >= 0) {
                ...
                if (code.equals("004")) {
                    ...
                    socket.close();
                    _inputThread = null;
                    throw new IrcException("Could not log into the Chat server: " + line);
                }
            }
            this.setNick(nick);
    ...
    public final synchronized void reconnect() throws IOException, IrcException, NickAlreadyInUseException{
        if (getServer() == null) {
            throw new IrcException("Cannot reconnect to Chat server, no record to reconnect to");
        }
     ...
     * quitServer() method.  Providing the IRCClient was connected to an
     ...
     *  <p>
     * So when a IRCClient is run on a machine that does not run an ident server,
     ...
     * the register provided by calling getLogin() and then shut down immediately.
     ...
     * may require the bot to have operator activity_main_status on the channel.
     * For example, if the bot has operator activity_main_status, we can grant
     * operator activity_main_status to "Dave" on the #cs channel
     ...
     * may require the bot to have operator activity_main_status if the topic
     * is protected.
     * 
     * @param channel The channel on which to perform the mode change.
     ...
     * may require the bot to have operator activity_main_status in the channel.
     * 
     ...
     * When the IRCClient receives information for each channel, it will
     ...
     * 
     * @deprecated As of IRCClient 1.2.0, use {@link #onIncomingFileTransfer(DccFileTransfer)}
     */
     ...
     * 
     * @deprecated As of IRCClient 1.2.0, use {@link #onIncomingChatRequest(DccChat)}
     */
     ...
     * output if the IRCClient has had its verbose mode turned on by calling
     * setVerbose(true);
     * 
     ...
     * then calling the appropriate method in the IRCClient.  This method is
    ...
    protected void handleLine(String line) {
            ...
            if (exclamation > 0 && at > 0 && exclamation < at) {
                ...
                if (tokenizer.hasMoreTokens()) {
                    ...
                    if (code != -1) {
                        ...
                        // It must be a nick without register and hostname.
        ...
        if (command.equals("PRIVMSG") && line.indexOf(":\u0001") > 0 && line.endsWith("\u0001")) {
            ...
            // If we reach this point, then we've found something that the IRCClient
    ...
    /**
     * This method is called once the IRCClient has successfully connected to
     * the IRC server.
     *  <p>
     * The implementation of this method in the IRCClient abstract class
     * performs no actions and may be overridden as required.
     * 
     * @since IRCClient 0.9.6
     */
    protected void onConnect() {}
    
    
    /**
     * This method carries out the actions to be performed when the IRCClient
     * gets disconnected.  This may happen if the IRCClient quits from the
    ...
    /**
     * This method is called by the IRCClient when a numeric response
     * is received from the IRC server.  We use this method to
     * allow IRCClient to process various responses from the server
        ...
        if (code == RPL_LIST) {
            ...
            while (tokenizer.hasMoreTokens()) {
                ...
                if (nick.startsWith("@")) {
                    ...
                    // Some wibbly activity_main_status I've never seen before...
                    prefix = ".";
                }
     ...
     * will be &quot;<code>IRCClient #test :I Am King of Test</code>&quot;
     ...
     *  <p>
     * IRCClient implements the interface ReplyConstants, which contains
     ...
     * users in that channel. The IRCClient collects this information and
     ...
     * @param login The register of the person who sent the message.
    ...
    /**
     * This method is called whenever a private message is sent to the IRCClient.
     *  <p>
     * The implementation of this method in the IRCClient abstract class
     * performs no actions and may be overridden as required.
     *
     * @param sender The nick of the person who sent the private message.
     * @param login The register of the person who sent the private message.
     ...
     * @param login The register of the user that sent the action.
     ...
     * @param sourceLogin The register of the user that sent the notice.
     ...
     * @param login The register of the user who joined the channel.
     ...
     * @param login The register of the user who parted from the channel.
     ...
     * @param login The register of the user.
     ...
     * @param kickerLogin The register of the user who performed the kick.
     ...
     * @param sourceLogin The register of the user that quit from the server.
     ...
     * IRCClient joins a new channel and discovers its topic.
     *  <p>
     * The implementation of this method in the IRCClient abstract class
     * performs no actions and may be overridden as required.
     *
    ...
    /**
     * After calling the listChannels() method in IRCClient, the server
     ...
     * @param sourceLogin The register of the user that set the mode.
    ...
    /**
     * Called when a user (possibly us) gets granted operator activity_main_status for a channel.
     *  <p>
     * This is a type of mode change and is also passed to the onMode
     * method in the IRCClient class.
     *  <p>
     * The implementation of this method in the IRCClient abstract class
     * performs no actions and may be overridden as required.
     * 
     ...
     * @param sourceLogin The register of the user that performed the mode change.
    ...
    /**
     * Called when a user (possibly us) gets operator activity_main_status taken away.
     *  <p>
     * This is a type of mode change and is also passed to the onMode
     * method in the IRCClient class.
     *  <p>
    ...
    /**
     * Called when a user (possibly us) gets voice activity_main_status granted in a channel.
     *  <p>
     * This is a type of mode change and is also passed to the onMode
     * method in the IRCClient class.
     *  <p>
    ...
    /**
     * Called when a user (possibly us) gets voice activity_main_status removed.
     *  <p>
     * This is a type of mode change and is also passed to the onMode
     * method in the IRCClient class.
     *  <p>
     ...
     * @param sourceLogin The register of the user that sent the invitation.
    ...
    /**
     * This method used to be called when a DCC SEND request was sent to the IRCClient.
    ...
    /**
     * This method used to be called when a DCC CHAT request was sent to the IRCClient.
    ...
    /**
     * This method is called whenever a DCC SEND request is sent to the IRCClient.
     ...
     * in order to allow multiple files to be downloaded by the IRCClient
     * at the same time. 
     *  <p>
     ...
     * This abstract implementation responds with the IRCClient's _version string,
     ...
     * @param sourceLogin The register of the user that sent the VERSION request.
     ...
     * @param sourceLogin The register of the user that sent the PING request.
     ...
     * @param sourceLogin The register of the user that sent the TIME request.
     ...
     * @param sourceLogin The register of the user that sent the FINGER request.
     ...
     * the IRCClient has not been programmed to recognise.
     *  <p>
     * The implementation of this method in the IRCClient abstract class
     * performs no actions and may be overridden as required.
     * 
     ...
     * IRCClient class in response to notification of nick changes that apply
     * to us.
     * 
    ...
    /**
     * Sets the internal register of the Bot.  This should be set before joining
     * any servers.
     *
     * @param login The new register of the Bot.
     */
    ...
    /**
     * Gets the name of the IRCClient. This is the name that will be used as
     * as a nick when we try to join servers.
     *
     * @return The name of the IRCClient.
     */
     ...
     *  <p>
     * The nick returned by this method is maintained only by the IRCClient
    ...
    /**
     * Gets the internal register of the IRCClient.
     *
     * @return The register of the IRCClient.
     */
    ...
    /**
     * Gets the internal version of the IRCClient.
     *
     * @return The version of the IRCClient.
     */
    ...
    /**
     * Gets the internal finger message of the IRCClient.
     *
     * @return The finger message of the IRCClient.
     */
    ...
    /**
     * Returns whether or not the IRCClient is currently connected to a server.
     * The result of this method should only act as a rough guide,
     * as the result may not be valid by the time you act upon it.
     *
     * @return True if and only if the IRCClient is currently connected to a server.
     */
     ...
     * change this value in IRCClient.  All lines greater than this length
    ...
    /**
     * Returns the name of the last IRC server the IRCClient tried to connect to.
     * This does not imply that the connection attempt to the server was
     * successful (we suggest you look at the onConnect method).
     * A value of null is returned if the IRCClient has never tried to connect
     * to a server.
     * 
    ...
    /**
     * Returns the port number of the last IRC server that the IRCClient tried
     * to connect to.
     * This does not imply that the connection attempt to the server was
     * successful (we suggest you look at the onConnect method).
     * A value of -1 is returned if the IRCClient has never tried to connect
     * to a server.
     * 
     * @since IRCClient 0.9.9
     * 
     ...
     * "GB2312" for Chinese encoding.  If a IRCClient is currently connected
     * to a server, then it must reconnect before this change takes effect.
     * 
     * @since IRCClient 1.0.4
     * 
     * @param charset The new encoding charset to be used by IRCClient.
     * 
    ...
    /**
     * Returns the InetAddress used by the IRCClient.
     * This can be used to find the I.P. address from which the IRCClient is
     * connected to a server.
     * 
     * @since IRCClient 1.4.4
     * 
     ...
     * @since IRCClient 1.4.4
     * 
     * @return An array of port numbers that IRCClient can use to send DCC
     ...
     * @since IRCClient 1.4.4
     * 
     * @param ports The set of port numbers that IRCClient may use for DCC
     ...
     * same instance as this IRCClient. This may be useful if you are writing
     * a multiple server IRC bot that uses more than one instance of IRCClient.
     * 
     * @since IRCClient 0.9.9
     *
     * @return true if and only if Object o is a IRCClient and equal to this.
     */
    ...
    /**
     * Returns the hashCode of this IRCClient. This method can be called by hashed
     * collection classes and is useful for managing multiple instances of
     * PircBots in such collections.
     * 
     * @since IRCClient 0.9.9
     * 
     * @return the hash code for this instance of IRCClient.
     */
    public int hashCode() {
     ...
     * if you are using more than one IRCClient instance to achieve
     * multiple server connectivity. The format of
     * this String may change between different versions of IRCClient
     * but is currently something of the form
     * <code>
     *   Version{IRCClient x.y.z Java IRC Bot - www.jibble.org}
     *   Connected{true}
     *   Server{irc.dal.net}
     *   Port{6667}
     *   Password{}
    ...
    /**
     * Disposes of all thread resources used by this IRCClient. This may be
     * useful when writing bots or clients that use multiple servers (and
     * therefore multiple IRCClient instances) or when integrating a IRCClient
     * with an existing program.
     *  <p>
     * Each IRCClient runs its own threads for dispatching messages from its
     * outgoing message queue and receiving messages from the server.
     * Calling dispose() ensures that these threads are
     * stopped, thus freeing up system resources and allowing the IRCClient
     * object to be garbage collected if there are no other references to
     * it.
     *  <p>
     * Once a IRCClient object has been disposed, it should not be used again.
     * Attempting to use a IRCClient that has been disposed may result in
    ...
    // Connection stuff.
    public InputThread _inputThread = null;
    public OutputThread _outputThread = null;
    public String _charset = null;
    public InetAddress _inetAddress = null;
    ...
    public Queue _outQueue = new Queue();
    public long _messageDelay = 1000;
    ...
    public Hashtable _channels = new Hashtable();
    
    // A Hashtable to temporarily store channel topics when we join them
    // until we find out who set that topic.
    public Hashtable _topics = new Hashtable();
    
    // DccManager to process and handle all DCC events.
    public DccManager _dccManager = new DccManager(this);
    public int[] _dccPorts = null;
    public InetAddress _dccInetAddress = null;
    
    // Default settings for the IRCClient.
    public boolean _autoNickChange = false;
    public boolean _verbose = false;
    public String _name = "";
    public String _nick = _name;
    public String _login = "";
    public String _version = "";
    public String _finger = "What is a finger, FR tho?";

    public String _channelPrefixes = "#&+!";
}
